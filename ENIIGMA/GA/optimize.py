...
cl = 2.99792458e08  # speed of light [m/s]
...
def ENIIGMA(
    ...
    popsize=50,
    fitness="rmse",
    initializator=Initializators.G1DListInitializatorReal,
    mutator=Mutators.G1DListMutatorRealGaussian,
    crossover=Crossovers.G1DBinaryStringXTwoPoint,
    scaling=Scaling.LinearScaling,
    ...
    if skip == False:
        ...
        for index0 in range(i_var):
            ...
            if bool(res) == True:
                ...
                tscore = np.loadtxt(
                    DIR + "Workspace/Interp/" + "comb_score0.txt"
                ).T  # pd.read_csv(DIR+'Workspace/Interp/'+'comb_score0.txt',sep='\s+', header=None)
                score = tscore[0]
        ...
        sp = np.loadtxt(
            DIR + "Workspace/Interp/Best_comb_selected.csv",
            dtype=str,
            delimiter=",",
            usecols=[matrix],
            skiprows=1,
        ...
        for i in range(matrix):
            try:
                ...
                print(" ")
                print(
                    "ENIIGMA info: No file was selected for the next step. Please, check the cuttof value."
                )
                print(" ")
        ...
        for index0 in range(i_var):
            ...
            if listmut[index0] in list2:
                ...
                tscore = np.loadtxt(
                    DIR + "Workspace/Interp2/" + "comb_score0.txt"
                ).T  # pd.read_csv(DIR+'Workspace/Interp/'+'comb_score0.txt',sep='\s+', header=None)
                score = tscore[0]
                count0 = count0 + 1
        ...
        df = df[df.best_chi <= 1.0 * cuttof]
        df.to_csv("Best_comb_selected.csv", index=False)

        sp = np.loadtxt(
            DIR + "Workspace/Interp2/Best_comb_selected.csv",
            dtype=str,
            delimiter=",",
            usecols=[matrix],
            skiprows=1,
        ...
        df = pd.read_csv(
            DIR + "Workspace/Processing/Interp_proc/All_R.txt", sep="\s+", header=None
        )
        # print df.shape
        # exit()
        q = df.shape[1] / 2
        ...
        if group_comb <= q:
            ...
            combin = combinations(
                qq, group_comb
            )  # number is the group size for each combination
            count = 0
            all = list(combin)
            ...
            def func2(all):
                ...
                for k in range(len(all)):
                    ...
                    tscore = np.loadtxt(
                        DIR + "Workspace/Processing/Interp_proc/" + "comb_score0.txt"
                    ).T  # pd.read_csv(DIR+'Workspace/Interp/'+'comb_score0.txt',sep='\s+', header=None)
                    score = tscore[0]
                    count = count + 1
            ...
            print(" ")
            print(
                "ENIIGMA info: ERROR: Cannot combine"
                + " "
                + str(q)
                + " "
                + "files in groups of"
            ...
            print(
                "ENIIGMA info: Please, use group_comb ="
                + " "
                + str(q)
                + " "
                + "or less."
        ...
        plt.plot(ntt, 0.0 * new_tau, ":", color="gray")
