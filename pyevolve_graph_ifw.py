#!/opt/anaconda3/envs/jwst/bin/python
...
def graph_diff_raw(pop, minimize, filesave=None):
    ...
    plt.annotate(
        "Maximum (%.2f)" % (diff_raw_max,),
        xy=(gen_max_raw, diff_raw_max),
        xycoords="data",
        xytext=(-150, -20),
    ...
    plt.annotate(
        "Maximum (%.2f)" % (diff_fit_max,),
        xy=(gen_max_fit, diff_fit_max),
        xycoords="data",
        xytext=(-150, -20),
        textcoords="offset points",
        arrowprops=dict(arrowstyle="->", connectionstyle="arc"),
...
def graph_maxmin_raw(pop, minimize, filesave=None):
    ...
    plt.annotate(
        "Min StdDev (%.2f)" % (min_std,),
        xy=(gen_min_std, min_std),
        xycoords="data",
        xytext=(8, 15),
        textcoords="offset points",
        arrowprops=dict(arrowstyle="->", connectionstyle="arc"),
    ...
    plt.annotate(
        "Max StdDev (%.2f)" % (max_std,),
        xy=(gen_max_std, max_std),
        xycoords="data",
        xytext=(8, 15),
        textcoords="offset points",
        arrowprops=dict(arrowstyle="->", connectionstyle="arc"),
...
def graph_maxmin_fitness(pop, minimize, filesave=None):
    ...
    plt.title(
        "Plot of evolution identified by '%s' (fitness scores)" % (options.identify)
    )
    plt.grid(True)
    plt.legend(prop=FontProperties(size="smaller"))
    ...
    for it_out in pop:
        ...
        if minimize:
            ...
            plt.plot(
                ...
                min_y,
                colors_list[index],
                label="Fitness min (%s)" % (id_list[index],),
                linewidth=1.3,
            )
        else:
            plt.plot(
                ...
                max_y,
                colors_list[index],
                label="Fitness max (%s)" % (id_list[index],),
                linewidth=1.3,
            )
            plt.plot(x, min_y, colors_list[index], linewidth=0.05)
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--identify",
        dest="identify",
        help="The identify of evolution.",
        metavar="IDENTIFY",
    )
    ...
    group.add_option(
        "-4",
        action="store_true",
        help="Max/min/avg graph (fitness scores).",
        dest="maxmin_fitness",
    )
    group.add_option(
        "-5",
        action="store_true",
        help="Raw and Fitness min/max difference graph.",
        dest="diff_raw",
    )
    ...
    group.add_option(
        ...
        "parameter, like 'one, two, three'), the maximum is 6 items.",
        dest="compare_raw",
    )
    group.add_option(
        "-7",
        ...
        "like 'one, two, three'), the maximum is 6 items.",
        dest="compare_fitness",
    )
    ...
    if options.identify and (
        not options.errorbars_raw
    ...
    if options.pop_heatmap_raw or options.pop_heatmap_fitness:
        ...
        if options.genrange:
            ...
            ret = c.execute(
                "select distinct generation from population where identify = ?",
                (options.identify,),
            )
        ...
        if len(generations) <= 0:
            print(
                "No generation data found for the identify '%s' !" % (options.identify,)
            )
            exit()
        ...
        if len(pop) <= 0:
            print(
                "No statistic data found for the identify '%s' !" % (options.identify,)
            )
            exit()
    ...
    if len(identify_list) == 1 and not popGraph:
        ...
        if options.genrange:
            genrange = options.genrange.split(":")
            ret = c.execute(
                "select * from statistics where identify = ? and generation between ? and ?",
                (options.identify, genrange[0], genrange[1]),
            )
        else:
            ret = c.execute(
        ...
        if len(pop) <= 0:
            print(
                "No statistic data found for the identify list '%s' !"
                % (options.identify,)
            )
            exit()
